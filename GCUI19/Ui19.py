...
class Uno(QThread):
    ...
    def run(self):
        ...
        for line in open('Data.csv','r'):
            ...
            #if i == 1:
                #ser.write(b'Command a')
                
            #elif i == 15:
                #ser.write(b'Command x')
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.graphicsView.setLabel('left', 'ADC',color='#000000', **{'font-size':'20pt'})
        self.graphicsView.setLabel('right', 'Temperature (C)',color='#000000', **{'font-size':'20pt'})
        self.graphicsView.setLabel('bottom', 'Time (Sec)',color='#000000', **{'font-size':'20pt'})

        self.graphicsView.getAxis("bottom").setStyle(tickFont = font)
        self.graphicsView.getAxis("left").setStyle(tickFont = font)
        self.graphicsView.getAxis("right").setStyle(tickFont = font)
        self.graphicsView.getAxis("top").setStyle(tickFont = font1)
        
        #self.graphicsView.setYRange(0, 1200)
        #self.graphicsView.setLabel('bottom','Time', units='Sec', **{'font-size':'20pt'})
        
        
        # Show Axis Right and Top
        #--------------------------------------------------
        ...
        #barItem = pg.BarGraphItem(x = tx, height = ty, width = 3, brush=(202,230,242))
        #self.graphicsView.addItem(barItem)
        #self.graphicsView.showAxis('top') 
        #self.graphicsView.getAxis('top').setTicks([[(i, Ticks[i-1]) for i in tx]])
        #self.graphicsView.showAxis('top') 
        #self.graphicsView.getAxis('top').setTicks([[(175, Ticks[0])]])
        ...
        #----------------------------------------------------
        self.pushButton.clicked.connect(lambda:self.buttonClick())
        #----------------------------------------------------
    ...
    def buttonClick(self):
        self.thread = Uno()
        self.thread.signal.connect(self.callback) #連線回撥函數，接收結果
        self.thread.start() # 啟動執行緒
        

    def callback(self,msg):
        A=[]
        #self.textBrowser.insertPlainText(str(msg[0:10]))
        #A=str(msg)
        A=float(msg)
        print(A)
        ...
        barItem = pg.BarGraphItem(x = tx, height = ty, width = 5, brush=(204,238,247),pen=(255,255,255))
        self.graphicsView.addItem(barItem)
        ...
        self.graphicsView.showAxis('top') 
        self.graphicsView.getAxis('top').setTicks([[(tx[0], Ticks[0]),(tx[1], Ticks[1]),(tx[2], Ticks[2])]])
        #self.graphicsView.getAxis('top').setTicks([[((i+1), Ticks[i+1]) for i in tx]])
        
        #-------------------------------------------------
        self.graphicsView.setYRange(50, 1200)
        pen = pg.mkPen(color='r', width=3) # DotLine
        self.graphicsView.plot(x1, data1, pen=pen)        
        #--------------------------------------------------
